...
bool DownloadThread::_openAndPrepareDevice()
{
    ...
    if (authopenresult == _file.authOpenCancelled) {
        ...
        msg += "<br>"+tr("Please verify if 'Aliro Ed Imager' is allowed access to 'removable volumes' in privacy settings (under 'files and folders' or alternatively give it 'full disk access').");
    ...
    switch (ret)
    {
        ...
        case CURLE_WRITE_ERROR:
            ...
            if (_file.errorCode() == ERROR_ACCESS_DENIED)
            {
                ...
                {
                    msg += "<br>"+tr("Controlled Folder Access seems to be enabled. Please add both aliro-imager.exe and fat32format.exe to the list of allowed apps and try again.");
                }
